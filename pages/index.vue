<template>
  ...
  <div id="container" class="absolute text-white text-center w-full max-w-2xl px-6"
    ...
    "
    >
      <h1 id="qujuanMiller" class="font-tomorrow text-xl mt-10 opacity-0" style="transform: translateY(50px);">
        <span>Q</span>
        <span>U</span>
        <span>J</span>
        <span>U</span>
      ...
      <hr class="rounded opacity-0"/>
      <p id="oneWithAn" class="font-museo text-3xl tracking-wide mt-2 opacity-0">ONE WITH AN EVERLASTING DESIRE FOR LEARNING AND KNOWLEDGE</p>
      <div class="container">
        <a href="/" id="btnViewWork" class="button mt-8 opacity-0" style="margin-left: auto; margin-right: auto; transform: translateY(-60px);">
          <div class="plate"></div>
          <div class="plate"></div>
          <div class="plate"></div>
          <div class="plate"></div>
...
function animate() {
  ...
  if (intersects.length > 0) {
    ...
    //vertice 1
    color.setX(intersects[0].face.a, 0.76);
    color.setY(intersects[0].face.a, 0.13);
    color.setZ(intersects[0].face.a, 0.12);

    // vertice 2
    color.setX(intersects[0].face.b, 0.76);
    color.setY(intersects[0].face.b, 0.13);
    color.setZ(intersects[0].face.b, 0.12);

    //vertice 3
    color.setX(intersects[0].face.c, 0.76);
    color.setY(intersects[0].face.c, 0.13);
    color.setZ(intersects[0].face.c, 0.12);
  ...
  stars.rotation.x += 0.00075;
  stars.rotation.y += 0.00075;
  // stars.rotation.z += 0.00075
}
...
document.querySelector('#btnViewWork').addEventListener('click', (e) => {
  e.preventDefault()
  gsap.to('#container', {
    opacity: 0
  })
...
.button__wrapper{
    ...
    transform : translate3d( 0px , 27px , 74px) ;
    perspective : 600px ;
}
.button__text{
    position : relative ;
...
hr {
  ...
  width: 100%;
  height: 0;
  border-top: 0.3rem dotted rgb(255, 255, 0);
  position: relative;
  overflow: visible;
  margin: 2rem 0;
}
